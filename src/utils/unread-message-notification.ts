...
const prepareUnreadMessageEmail = (data: UnreadMessageEmailData, locale: string = 'en'): string => {
  ...
  const translations = {
    en: {
      subject: 'New Message on DealOnIt',
  ...
  const appUrl = process.env.NEXT_PUBLIC_URL || 'https://dealonit.com'
      ...
      <div class="email-container">
        ...
        <div class="footer">
          <p style="margin-bottom: 5px;">
            &copy; ${year} DealOnIt.com. ${isRtl ? 'جميع الحقوق محفوظة' : 'All rights reserved'}
          </p>
...
const sendUnreadMessageNotification = async (data: UnreadMessageEmailData, locale: string = 'en') => {
  ...
  try {
    ...
    const translations = {
  en: { subject: 'New Message on DealOnIt' },
  ar: { subject: 'رسالة جديدة على DealOnIt' }
    }
